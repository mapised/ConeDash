...
function Block:new(id, args, x, y, rotation)
    Block.super.new(self, id, args, x, y, rotation)
...
function Block:enter()
    if (gameState.player.y * gameState.player.gravity) < self.y then
        gameState.player.y = self.y + (((-(self.height * constants.blockSize) / 2) - (constants.blockSize / 2) + 1) * gameState.player.gravity)
...
function Block:draw()
    ...
    if self.height == 0.5 then
        local r, g, b, a = love.graphics.getColor()
        love.graphics.setColor(0, 0, 0)
        love.graphics.draw(sprites.slabColor, self.x, self.y, self.rotation, width, height, self.sprite:getWidth() / 2, self.sprite:getHeight() / 2)
